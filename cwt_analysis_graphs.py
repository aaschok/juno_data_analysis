...
from juno_classes import CWTData, MagData, PlotClass, PosData, Turbulence
from juno_functions import (find_orb_num, get_orbit_data, get_sheath_intervals,
                            time_in_lat_window)
...
def _plots(start_datetime, end_datetime, window_size, days_per_graph,
    ...
    print(end_datetime)
    mag_class = MagData(start_datetime.isoformat(
    ), end_datetime.isoformat(), instrument=['fgm_jno', 'r60s'])
    mag_class.downsample_data(60)
    mag_class.cart_to_sphere()
    mag_class.mean_field_align(window_size=window_size)
    mag_data = mag_class.data_df[['BX', 'BY', 'BZ',
                                  'B_PERP1', 'B_PERP2', 'Br', 'Btheta', 'Bphi']]
    sheath_windows_df = get_sheath_intervals(
        '/data/juno_spacecraft/data/crossings/crossingmasterlist/jno_crossings_master_v6.txt')
    ...
    num_graphs = (end_datetime - start_datetime) / \
        pd.Timedelta(days=days_per_graph)
    ...
    for i in range(0, int(np.floor(num_graphs)) + 1):
        ...
        b_perp = np.sqrt(graph_data.B_PERP1**2 +
                         graph_data.B_PERP2**2).to_numpy()
        cwt = CWTData(graph_data.index, b_perp, 60,
        ...
        fig.suptitle(
            f'{start_datetime.date()} to {end_datetime.date()}', fontsize=20)
        ...
        ax_sys3.set_title(
            f'Distance Range {round(sys_3_df.radial_3.loc[start_datetime.isoformat()],2)} $R_j$ - {round(sys_3_df.radial_3.loc[end_datetime.isoformat()],2)} $R_j$')
        ...
        try:
            with open('/home/delamere/Juno/jad_mean_orbit_{orb_num}.pkl', 'rb') as jad_mean_file:
                ...
                jad_mean_file.close()
            plot_index = np.where((mean_data.jad_tm >= start_datetime) & (
                mean_data.jad_tm <= end_datetime))
            ax0.plot(mean_data.jad_tm[plot_index],
                     mean_data.jad_mean[plot_index], label='mean_counts')
        except:
            pass
        ax0.legend()
        ...
        ax2.axvline(1/timedelta(hours=5).total_seconds(),
                    color='gray', linestyle='--')
        ax2.axvline(1/timedelta(hours=10).total_seconds(),
                    color='gray', linestyle='--')
        ...
        cwt_data = {'Time': cwt.time_series,
                    'Data': cwt.power, 'Freq': cwt.freqs}
...
def sheath_cwt():
    sheath_windows_df = get_sheath_intervals(
        '/data/juno_spacecraft/data/crossings/crossingmasterlist/jno_crossings_master_v6.txt')
    min_frequency = 1/timedelta(hours=20).total_seconds()
    max_frequency = 1/timedelta(hours=1).total_seconds()
    days_per_graph = 3
    ...
    for index, row in sheath_windows_df.iterrows():
        ...
        mag_class.downsample_data(60)
        mag_class.cart_to_sphere()
        mag_class.mean_field_align(window_size=24)
        mag_data = mag_class.data_df[[
            'BX', 'BY', 'BZ', 'B_PERP1', 'B_PERP2', 'Br', 'Btheta', 'Bphi']]

        b_perp = np.sqrt(mag_data.B_PERP1**2 + mag_data.B_PERP2**2).to_numpy()
        cwt = CWTData(mag_data.index, b_perp, 60, min_frequency, max_frequency)
        try:
            fig = plt.figure(figsize=(20, 10))
            grid = gs.GridSpec(2, 2, figure=fig)

            ax0 = fig.add_subplot(grid[0, 0])
            ax0.plot(mag_data.index, b_perp)
            ...
            save_location = '/home/aschok/Documents/figures/cwt/sheath'
            start_name = row.START.strftime('%Y%j')
            end_name = row.END.strftime('%Y%j')
            save_name = f'sheath_cwt_{start_name}-{end_name}.png'
            plt.savefig(f'{save_location}/{save_name}')
            plt.close(fig)

            psd_df = pd.DataFrame({'freqs': cwt.freqs, 'psd': cwt.psd})
            pickle_file = f'sheath_psd_{start_name}-{end_name}.pkl'
            with open(f'{save_location}/psd/{pickle_file}', 'wb') as pkl:
...
def sheath_psd_hist():
    psd_directory = '/home/aschok/Documents/figures/cwt/sheath/psd'
    ...
    for parent, child, files in os.walk(psd_directory):
        for file in files:
            if file.endswith('.pkl'):
                ...
                continue
            with open(f'{psd_directory}/{file}', 'rb') as pkl:
                psd = pickle.load(pkl)

            psd = psd[(min_f < psd.freqs)
                      & (max_f > psd.freqs)]
            peaks, _ = signal.find_peaks(np.log10(psd.psd))
...
def find_disturbed(start, end, window_size, major, minor, min_freq, max_freq, save):
    ...
    def _disturbed_plot(start_datetime, end_datetime, window_size, major, minor, min_frequency, max_frequency,
        ...
        plot_class = PlotClass(ax0, ylabel='Field (nT)',
                               title='Perpendicular Field')
        ...
        ax2.axhline([np.mean(per_freq)], linestyle='--',
        ...
        print(
            f'Disturbed regions for {start_datetime.date()} to {end_datetime.date()} plotted')
        plt.close(fig)
    ...
    for index, row in lat_df.iterrows():
        ...
        print(start.date())
        print(end.date())
        _disturbed_plot(start, end, window_size, major,
                        minor, min_freq, max_freq, save)
...
def psd_compare(start, end, window_size, min_freq, max_freq, save):
    ...
    for index, row in lat_df.iterrows():
        ...
        fig, ((cax1, pax1), (cax2, pax2), (cax3, pax3)) = plt.subplots(3, 2, figsize=(18, 10),
                                                                       sharex='col')
        fig.suptitle(
            'Comparison of PSDs above, below, and without a threshhold')

        cwt.cwt_plot(cax1, False, False, colorbar=True)
        cwt.psd_plot(pax1, 'hour', ylabel='PSD', title='No Threshhold')

        mean = np.mean(cwt.power)
        above_peak_matrix = np.ma.masked_less_equal(
            cwt.power, mean).filled(fill_value=0)
        below_peak_matrix = np.ma.masked_greater_equal(
            cwt.power, mean).filled(fill_value=0)
...
def heating_distributions(start_iso, end_iso):
    ...
    def _calc_func(start_datetime, end_datetime):
        ...
        et_range = [spice.utc2et(i)
                    for i in time_series.strftime('%Y-%m-%dT%H:%M:%S')]

        positions, lt = spice.spkpos(
            'JUNO', et_range, 'JUNO_JSS', 'NONE', 'JUPITER')
        x = positions.T[0]
        y = positions.T[1]
        z = positions.T[2]
    ...
    for index, row in lat_df.iterrows():
        ...
        local_time = np.append(
            local_time, (np.mean(longitude) + 180)*24/360 % 24)
        mean_q = np.append(mean_q, np.mean(q_kaw))

        ax2.scatter(np.array(radial_distance),
                    q_kaw, marker='+', color='black')
    ...
    ax1.set_yscale('log')
    plt.savefig('/home/aschok/Documents/figures/test3.png',
                bbox_inches='tight')
